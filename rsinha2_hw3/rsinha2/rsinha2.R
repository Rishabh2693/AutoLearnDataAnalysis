...
###
testIdx <- setdiff(seq.int(nrow(iris)),trainIdx)
...
# create contingency table of predictions against ground truth
###
table(NNpreds, factor(iris[testIdx,ncol(iris)]))
...
myknn <- function(train, test, cl, k)
{
    ...
    for(i in 1:nrow(test))
    {
        ...
        for(j in 1:nrow(train))
        {
            ...
            # dist function
            ###
	    dists[j] <- sum(abs(train[j,]-test[i,]))	
        }
        ...
        ###
	kDist = head(sort(dists),k)
	indx <- match(dists,kDist)
	idx <- which(!is.na(indx))
	
	#which.max handles ties by returning the first value in the list
	if(is.element(names(which(table(cl[idx])==max(table(cl[idx])))),"setosa"))
	        classes[i] <- "setosa"
	else if(is.element(names(which(table(cl[idx])==max(table(cl[idx])))),"versicolor"))
